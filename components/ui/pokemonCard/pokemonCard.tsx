...
export default async function PokemonCard({ pokemon }: { pokemon: any }) {
    ...
    return (
        <div className={`flex min-w-80 ${bgColor} rounded-lg shadow-md shadow-slate-700 cursor-pointer`}>
            <div className="flex flex-col gap-2 justify-center items-center w-4/5">
                ...
                <div className="flex justify-center w-4/5 gap-1 ml-2">
                    {pokemon.types.map((e: any, index: number) => {
                        ...
                        return <div className="text-white text-sm border-2 rounded-lg w-full text-center" key={index}>{typeUpperCase}</div>
                    })}
                </div>
            </div>
            <Image src={pokemon.sprites.front_default} width={100} height={100} alt="Foto do pokemon" className="w-2/5 bg-opacity-50 bg-slate-300 rounded-l-full" />
        </div >
    )
}